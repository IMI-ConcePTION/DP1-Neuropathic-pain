...
for (algo in c("V3")) {
  ...
  for (dr in c("gaba","pregaba","any")) { 
    smart_load(paste0("D3_components_pregnancy_V3","_",dr,"_validation_FISABIO"), dirtemp, extension = "rds")
    ...
    y_end<-as.character(get(paste0("D3_components_pregnancy_V3","_",dr))[,max(year(pregnancy_start_date))])
    
    copy_tmp<-copy(get(paste0("D3_components_pregnancy_V3","_",dr,"_validation_FISABIO")))
    ...
    #FISABIO has not components B and C 
