...
for (algo in c("M1","S1","S2","S3","S4","S5","S6")) {
  ...
  for (dr in c("gaba","pregaba","any")) { 
    ...
    smart_load(paste0("D3_components_pregnancy_",algo,"_",dr), dirtemp, extension = "rds")
    ...
    y_end<-as.character(get(paste0("D3_components_pregnancy_",algo,"_",dr))[,max(year(pregnancy_start_date))])
    
    copy_tmp<-copy(get(paste0("D3_components_pregnancy_",algo,"_",dr)))
    copy_tmp[,Year_at_pregnancy_start:=paste0("2006-",y_end)]
    
    assign(paste0("D3_components_pregnancy_",algo,"_",dr),get(paste0("D3_components_pregnancy_",algo,"_",dr))[,Year_at_pregnancy_start:= fifelse(year(pregnancy_start_date)>=2006 & year(pregnancy_start_date)<=2012 , "2006-2012", paste0("2013-",y_end))])
    
    assign(paste0("D3_components_pregnancy_",algo,"_",dr),rbind(get(paste0("D3_components_pregnancy_",algo,"_",dr)),copy_tmp))
    
#Create column indicationB_C, containing the info on components B and C which has the priority over all the others  --------------
  ...
  #create indication2, including information on components from D to I -------------------      
    ...
    #OLD
    # assign(paste0("D3_components_pregnancy_",algo,"_",dr),get(paste0("D3_components_pregnancy_",algo,"_",dr))[!is.na(indicationB_C),indicationB_C:=1][is.na(indicationB_C),indicationB_C:=0][,indicationB_C:=max(as.numeric(indicationB_C)),by=c("pregnancy_id","Year_at_pregnancy_start")][indicationB_C==1,indication2:="NULL"])
...
#create variable indication for daps with no primaty care, fill it with indication2. Complete variable indication, which is filled for DAPs with component B and/or C)
    ...
    #suppressWarnings(get(paste0("D3_components_pregnancy_",algo,"_",dr))[sum>1,indicationfinal:="more than one"][sum==1,indicationfinal:=indication,by=c("pregnancy_id","Year_at_pregnancy_start")][,indication:=NULL])
