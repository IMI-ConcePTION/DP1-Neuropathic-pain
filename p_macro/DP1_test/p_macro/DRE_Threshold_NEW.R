DRE_Threshold <- function(Inputfolder, Outputfolder,Intermediatefolder=NULL, Varlist = NULL, Delimiter = ",", NAlist = NULL, FileContains = NULL){
  ...
  for (file_name in files_names) {
    ...
    if(any(colnames(File) %in% names(Varlist))) {
      ...
      for(x in 1:length(Varlist_in_file)) {
        ...
        flag_name<-paste0(Var_in_file,"_flag")
        if (!is.null(Intermediatefolder)) File[, (Var_in_file) := as.character(get(Var_in_file))][as.integer(get(Var_in_file)) < Varlist_in_file[[x]] & as.integer(get(Var_in_file)) > 0,(flag_name) :="x"]
 
      }
      
      
      if (!is.null(Intermediatefolder)) {
        intermediate_file_path <- file.path(Intermediatefolder, file_name)
        fwrite(File, file = intermediate_file_path, sep = Delimiter, col.names = T, row.names = F, na = "", append = F)
        flag_name<-paste0(names(Varlist_in_file),"_flag")
        File[,(flag_name) :=NULL]
      }
