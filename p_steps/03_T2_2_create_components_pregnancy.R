...
#ALGO M1: Main analyses for combining the components results 
  ...
  for (i in 1:length(files)) {
    if (str_detect(files[i],"^VISIT_OC"))  {
      visit_tmp<-unique(fread(paste0(dirinput,files[i],".csv"))[,.(person_id,visit_occurrence_id,meaning_of_visit)], colClasses = list( character="person_id"))
...
setnames(meanings_event,"meaning_of_event","meaning_renamed")
  ...
  for (event in events_of_interest) {
    ...
    assign(event,merge(get(event),unique(VISIT_OCCURRENCE[,.(visit_occurrence_id,meaning_of_visit)]),all.x=T,by="visit_occurrence_id"))
  ...
  for (dr in c("gaba","pregaba","any")) {
    ...
    if (thisdatasource=="THL") {
      ...
      get(paste0("D3_indication_SAP1_pregnant_woman_",dr))[indication_code %in% c(111, 181, 182, 183, 199) , component_B:="epilepsy"]
    }
    ...
    if (thisdatasource=="EFEMERIS") {
      ...
      get(paste0("D3_indication_SAP1_pregnant_woman_",dr))[prescriber_speciality==14 |prescriber_speciality==2 | prescriber_speciality==41 , component_C:="NEUROPATHIC_PAIN"]
      
      get(paste0("D3_indication_SAP1_pregnant_woman_",dr))[prescriber_speciality==33 |prescriber_speciality==75 , component_C:="GAD"]
    }
    
    
    if (thisdatasource=="THL") {
      print("Computing component C")
      get(paste0("D3_indication_SAP1_pregnant_woman_",dr))[prescriber_speciality==570 |prescriber_speciality==742 | prescriber_speciality==100 |prescriber_speciality==202 | prescriber_speciality==230  , component_C:="NEUROPATHIC_PAIN"]
    }
    
    
    if (thisdatasource=="UOSL") {
      print("Computing component C")
      get(paste0("D3_indication_SAP1_pregnant_woman_",dr))[prescriber_speciality==41 |prescriber_speciality==2 | prescriber_speciality==84 |prescriber_speciality==152 | prescriber_speciality==192 , component_C:="NEUROPATHIC_PAIN"]
      
      get(paste0("D3_indication_SAP1_pregnant_woman_",dr))[prescriber_speciality==58 , component_C:="GAD"]
    }
    ...
    if (thisdatasource=="SNDS") {
      ...
      get(paste0("D3_indication_SAP1_pregnant_woman_",dr))[prescriber_speciality==33 |prescriber_speciality==17 , component_C:="GAD"]
    }
    ...
    if (thisdatasource=="THL" | thisdatasource=="SAIL Databank") {
      ...
      diagnosis_exist<-diagnosis_exist[ ,component_Ddetailed:=list(list(unique(component_D))),by=c("pregnancy_id","date")]
      diagnosis_exist[count_rows_perid>1,component_D:="more than one"]
      diagnosis_exist<-unique(diagnosis_exist[,count_rows_perid:=NULL],by=c("pregnancy_id","date","component_D"))
    ...
    #COMPONENT F ------------------
    if (thisdatasource!="EFEMERIS"  & thisdatasource!="RDRU_FISABIO") {
      ...
      diagnosis_exist[,count_rows_perid:=.N,by=c("pregnancy_id","date")]
      diagnosis_exist<-diagnosis_exist[ ,component_Fdetailed:=list(list(unique(component_F))),by=c("pregnancy_id","date")]
      diagnosis_exist[count_rows_perid>1,component_F:="more than one"]
      diagnosis_exist<-unique(diagnosis_exist[,count_rows_perid:=NULL],by=c("pregnancy_id","date","component_F"))
    ...
    #COMPONENT G ------------------
    if (thisdatasource=="THL"  | thisdatasource=="ARS" | thisdatasource=="SAIL Databank" | thisdatasource=="RDRU_FISABIO" ) {
      ...
      diagnosis_exist[,count_rows_perid:=.N,by=c("pregnancy_id","date")]
      diagnosis_exist<-diagnosis_exist[ ,component_Gdetailed:=list(list(unique(component_G))),by=c("pregnancy_id","date")]
      diagnosis_exist[count_rows_perid>1,component_G:="more than one"]
      diagnosis_exist<-unique(diagnosis_exist[,count_rows_perid:=NULL],by=c("pregnancy_id","date","component_G"))
    ...
    if (thisdatasource=="THL" | thisdatasource=="ARS" | thisdatasource=="FERR" ) {
      ...
      diagnosis_exist[,count_rows_perid:=.N,by=c("pregnancy_id","date")]
      diagnosis_exist<-diagnosis_exist[ ,component_Fbisdetailed:=list(list(unique(component_Fbis))),by=c("pregnancy_id","date")]
      diagnosis_exist[count_rows_perid>1,component_Fbis:="more than one"]
      diagnosis_exist<-unique(diagnosis_exist[,count_rows_perid:=NULL],by=c("pregnancy_id","date","component_Fbis"))
    ...
    if (thisdatasource=="ARS" | thisdatasource=="SNDS" | thisdatasource=="FERR") {
      ...
      diagnosis_exist[,count_rows_perid:=.N,by=c("pregnancy_id","date")]
      diagnosis_exist<-diagnosis_exist[ ,component_Hdetailed:=list(list(unique(component_H))),by=c("pregnancy_id","date")]
      diagnosis_exist[count_rows_perid>1,component_H:="more than one"]
      diagnosis_exist<-unique(diagnosis_exist[,count_rows_perid:=NULL],by=c("pregnancy_id","date","component_H"))
    ...
    #COMPONENT I------------------
      ...
      # if(thisdatasource=="RDRU_FISABIO_old"){
      #   if(algo=="S2") {
      #     cond_drugs<-"date_disp>=date-allowed_window1 & date_disp<=date & !is.na(date_disp) & Meaning_of_drug_record=='Outpatient_pharmacy_management' & meaning_of_visit=='oupatient_specialist_visit'"
      #   }else{
      #     cond_drugs<-"date_disp>=date-allowed_window1 & date_disp<=date+allowed_window2 & !is.na(date_disp) & Meaning_of_drug_record=='outpatient_pharmacy_management' & meaning_of_visit=='oupatient_specialist_visit'"
      #   }       
      # }else{
      #   if(algo=="S2") {
    ...
    for (event in c("EPILEPSY","GAD","NEUROPATHIC_PAIN")) {
      ...
      if(thisdatasource=="RDRU_FISABIO_old")  {
        ...
        assign(event_complete,unique(get(event_complete)[,.(person_id,date_disp)]))
      }
  ...
  for (dr in c("gaba","pregaba","any")) {
    shell_base<-get(paste0("D3_indication_SAP1_pregnant_woman_",dr))[, .SD, .SDcols = patterns("^comp|person_id|pregnancy_|date")]
    ...
    smart_save(shell_base, dirtemp, override_name = paste0("D3_components_pregnancy_",algo,"_",dr), extension = "rds")
    ...
    fwrite(shell_base_csv, file=paste0(dirtemp,"D3_components_pregnancy_",algo,"_",dr,".csv"))
    ...
    fwrite(get(paste0("D4_proportion_per_component_pregnancy_",algo)),file=paste0(direxp,"D4_proportion_per_component_pregnancy_",algo,".csv"))
    ...
    # smart_save(tmp, dircheck, override_name = paste("D4_proportion_per_component_pregnancy",algo, "summary_levels", sep = "_"), extension = "csv")
    fwrite(tmp, file=paste0(dircheck,"D4_proportion_per_component_pregnancy_",algo, "_summary_levels",".csv"))
