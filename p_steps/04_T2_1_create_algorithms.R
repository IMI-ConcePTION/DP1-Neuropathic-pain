...
for (algo in c("M1","S1","S2","S3","S4","S5","S6")) {
  ...
  for (dr in c("gaba","pregaba","any")) {
    smart_load(paste0("D3_components_",algo,"_",dr), dirtemp, extension = "rds")

    y_end<-as.character(get(paste0("D3_components_",algo,"_",dr))[,max(year(first_disp))])
    ...
    copy_tmp<-copy(get(paste0("D3_components_",algo,"_",dr)))
    copy_tmp[,Year_first_disp:=paste0("2006-",y_end)]
    
    assign(paste0("D3_components_",algo,"_",dr),get(paste0("D3_components_",algo,"_",dr))[,Year_first_disp:= fifelse(year(first_disp)>=2006 & year(first_disp)<=2012 , "2006-2012", paste0("2013-",y_end))])
    
    assign(paste0("D3_components_",algo,"_",dr),rbind(get(paste0("D3_components_",algo,"_",dr)),copy_tmp))
    
    
    #start creation of indication variable
    if( thisdatasource=="SNDS" | thisdatasource=="UOSL" ) {
      get(paste0("D3_components_",algo,"_",dr))[!is.na(component_C),indication:=list(list(unique(component_C))),by=c("person_id","Year_first_disp")][,indicationB_C:=get("component_C")][is.na(get("indication")),indication:="NULL"][is.na(get("indicationB_C")),indicationB_C:="NULL"]
    ...
    #create indication2, including information on components from D to I ------------------- 
    comp_list<-list("D","E","F","Fbis","G","H","I")
    comp<-comp_list[[1]]
    ...
    #OLD
    # assign(paste0("D3_components_",algo,"_",dr),get(paste0("D3_components_",algo,"_",dr))[!is.na(indicationB_C),indicationB_C:=1][is.na(indicationB_C),indicationB_C:=0][,indicationB_C:=max(as.numeric(indicationB_C)),by=c("person_id","Year_first_disp")][indicationB_C==1,indication2:="NULL"])
  ...
  if(thisdatasource=="THL") {
    get(paste0("D4_proportion_per_algorithm_",algo))[Year_first_disp_LabelValue!="2006-2018",Year_first_disp_LevelOrder:=1]
  }else{
